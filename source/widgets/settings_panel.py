...
class Settings_Panel(AnchorLayout):
    ...
    def create_display_settings(self, *args) -> GridLayout:
        ...
        increase_new_packet_slider_range.background_color = (0.5, 0.5, 0.5, 0.8)
        increase_new_packet_slider_range.background_down = ""
        increase_new_packet_slider_range.size_hint_x = 0.1
        ...
        increase_ip_display_slider_range.background_color = (0.5, 0.5, 0.5, 0.8)
        increase_ip_display_slider_range.background_down = ""
        increase_ip_display_slider_range.size_hint_x = 0.1
        increase_ip_display_slider_range.size_hint_y = 1
        ...
        top_container = BoxLayout(orientation="vertical")
        ...
        ip_display_label_container.add_widget(Label(size_hint_x=0.7))

        top_container.add_widget(Label(size_hint_y=1))
        top_container.add_widget(ip_display_label_container)
        top_container.add_widget(Label(size_hint_y=0.2))
        ...
        top_container.add_widget(display_slider_container)
        top_container.add_widget(Label(size_hint_y=1))
    ...
    def init_data_summary_labels(self) -> None:
        ...
        reset_sniffer_button.text = "R"
        reset_sniffer_button.background_color = (.7, .7, .7, 0.8)
        reset_sniffer_button.font_size = sp(8)
        reset_sniffer_button.border = (0,0,0,0)
        reset_sniffer_button.size = (dp(20), dp(25))
        reset_sniffer_button.size_hint_x = 1


        self.switch_sniffer_button = Button()
        self.switch_sniffer_button.text = "N"
        self.switch_sniffer_button.background_color = (.7, .7, .7, 0.8)
        self.switch_sniffer_button.font_size = sp(8)
        self.switch_sniffer_button.border = (0,0,0,0)
        self.switch_sniffer_button.size = (dp(20), dp(25))
        self.switch_sniffer_button.pos = (dp(20),0)
        ...
        if self.gui_manager.connected_to_sniffer == True:
            ...
            connected_ip = self.gui_manager.config_variables_dict["last_connection"][0]
            server_name = self.gui_manager.config_variables_dict["stored_connections"][connected_ip][0]
            connected_port = self.gui_manager.config_variables_dict["last_connection"][1]

            self.connection_label.text = f"[color=#00ff00][b]Connected[/b][/color] to [color=#00ff00][b]{server_name}[/color][/b] on port [color=#00ff00][b]{connected_port}[/color][/b]"
    ...
    def select_interface(self, calling_button: Button) -> None:
        ...
        if return_msg.startswith(b"listening on"):
            ...
            self.interface_label.text = f"Interface: {calling_button.text}"
    ...
    def initalize_interface_dropdown(self):
        ...
        for key in self.gui_manager.interface_dictionary.keys():
            ...
            if key  == "OS":
                ...
                btn = Button(text=key)
                btn.background_color = (.7, .7, .7, 0.8)
                btn.font_size = sp(12)
                btn.border = (0,0,0,0)
    ...
    def create_about_panel(self) -> RelativeLayout:
        ...
        webbutton_container.add_widget(Label(size_hint_x=0.5))
        webbutton_container.pos = (dp(0), dp(5))
    ...
    def copy_database_location(self, calling_button: Button) -> None:
        ...
        if self.gui_manager.kivy_application.operating_system == "Darwin" and getattr(sys, "frozen", False):
            database_location = os.path.join( self.gui_manager.kivy_application.darwin_network_visualizer_dir, "sessions.sqlite")

        else:
            database_location = os.path.join(self.gui_manager.resource_path, "assets/database/sessions.sqlite")
    ...
    def connect_to_network_sniffer(self, connect_to_remote_server_button: Button) -> None:
        ...
        connection_name = connect_to_remote_server_button.text
        connection_name = connection_name.replace(" ", "")
        
        self.gui_manager.current_connection = connection_name
        self.gui_manager.current_connection_key = connect_to_remote_server_button.ip
        ...
        if self.gui_manager.server_socket == None:
            ...
            try:
                ...
                self.gui_manager.config_variables_dict["last_connection"][0] = ip
                self.gui_manager.config_variables_dict["last_connection"][1] = req_port
            ...
            server_name = self.gui_manager.config_variables_dict["stored_connections"][ip][0]
            self.connection_label.text = f"[color=#00ff00][b]Connected[/b][/color] to [color=#00ff00][b]{server_name}[/color][/b] on port [color=#00ff00][b]{req_port}[/color][/b]"
            ...
            self.gui_manager.transition.direction = "down"
            self.gui_manager.current = "loading"
            Clock.schedule_once(partial(self.gui_manager.kivy_application.switch_sniffer, [ip, req_port]), 0.75)
    ...
    def add_remote_connection(self, calling_button: Button) -> None:
        ...
        self.server_name_textinput.text = ""

        self.remote_connect_dropdown.children[0].clear_widgets()
        self.remote_connect_dropdown_short.children[0].clear_widgets()

        self.gui_manager.config_variables_dict["stored_connections"][ip] = [servername, port]

        for connection in self.gui_manager.config_variables_dict["stored_connections"].keys():

            ip = connection
            server_name = self.gui_manager.config_variables_dict["stored_connections"][connection][0]
            port = self.gui_manager.config_variables_dict["stored_connections"][connection][1]
        ...
        malicious_ips_database_name = servername +"_malicious_ips"
        cursor = db.cursor()
        cursor.execute(
    ...
    def create_sniffer_settings_panel(self):
        ...
        for connection in self.gui_manager.config_variables_dict["stored_connections"].keys():
            ...
            btn = Button(text=self.gui_manager.config_variables_dict["stored_connections"][connection][0])
            btn.ip = connection
            btn.port = self.gui_manager.config_variables_dict["stored_connections"][connection][1]
            btn.size_hint = (None, None)
            btn.width = dp(190)
            btn.height = dp(20)
            btn.background_color = (.8, .8, .8, 0.8)
        ...
        self.connect_to_network = Button(text="Connect to Network Sniffer", size_hint_x=1.7, background_color=[0.5, 0.5, 0.5, 0.8], border = (0,0,0,0))
        ...
        self.connect_to_interface_dropdown.add_widget(Button(text="Hello you!"))
        ...
        select_interface_container = BoxLayout(orientation="horizontal")
        select_interface_container.add_widget(Label(size_hint_x=0.1))
        select_interface_container.add_widget(self.interface_label)
        select_interface_container.add_widget(self.connect_to_interface)
        select_interface_container.add_widget(Label(size_hint_x=0.1))
        base.add_widget(select_interface_container)
