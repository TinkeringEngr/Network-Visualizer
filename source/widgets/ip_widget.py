...
class IP_Widget(Widget):
    ...
    def __init__(self, **kwargs):
        ...
        if self.country == "Local":
            self.whois_description.text = "Local"
    ...
    def tag(self, *args) -> None:
        ...
        if self.malicious_ip == False:  # check to see if already malciious
            ...
            self.malicious_timestamp = datetime.datetime.fromtimestamp(time_stamp).strftime("%d-%m-%Y")
            ...
            if self.id not in self.gui_manager.malicious_ips_local_database:
                ...
                cursor = db.cursor()
                malicious_ips_database_name = self.gui_manager.current_connection + "_malicious_ips"
                sql = f"""INSERT OR IGNORE INTO {malicious_ips_database_name} (ip, description, ban_lists, time_stamp, location_country,  packet_count, data_in, data_out, abuse_email ) VALUES ( "{self.id}", "{self.whois_description.text}", {self.banlist_count}, "{self.malicious_timestamp}", "{self.country}", {self.ip_data['packet_count']}, {self.ip_data['data_in']}, {self.ip_data['data_out']}, "{emails_string}"  ) """
    ...
    def make_display_menu(self) -> Scatter:
        ...
        copy_ip_address_button.background_color = (.3, .3, .3, 0.8)
