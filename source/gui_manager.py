...
class GUI_Manager(ScreenManager):
    ...
    def __init__(self, **kwargs):
        ...
        loading_string = Label(text="Loading...", font_size=sp(36))
        ...
        self.current_connection = None #current sniffer connection (type: string)
        ...
        if self.config_variables_dict["auto_connect"] == True and not kwargs["switch_sniffer_info"]:
    ...
    def switch_to_new_sniffer(self, switch_sniffer_info: tuple[str, str]) -> None:
        ...
        server_name = self.config_variables_dict["stored_connections"][switch_sniffer_info[0]][0] 
        self.current_connection_key = switch_sniffer_info[0]
        self.current_connection = server_name.replace(" ", "")
        ...
        try:
            ...
            self.config_variables_dict["last_connection"][0] = switch_sniffer_info[0]
            self.config_variables_dict["last_connection"][1] = switch_sniffer_info[1]
    ...
    def autoconnect_sniffer(self):
        ...
        sniffer_ip = self.config_variables_dict["last_connection"][0]
        server_name = self.config_variables_dict["stored_connections"][sniffer_ip][0]
        self.current_connection = server_name.replace(" ", "")
        self.current_connection_key = self.config_variables_dict["last_connection"][0]
        sniffer_port = self.config_variables_dict["last_connection"][1]
        sniffer_data_port = sniffer_port + 1
    ...
    def set_sniffer_state(self) -> None:
        ...
        # Position Computer GUI Widget
        if self.config_variables_dict["first_time_starting"] == True:

            screen_x, screen_y = self.calculate_mercator_coordinates( my_ip_info["longitude"], my_ip_info["latitude"] )
            self.my_computer.mercator_position = (screen_x, screen_y)
            self.my_computer.graph_position = (self.window_x/2 - dp(17), self.window_y/2  - dp(17)) 

        else:
            self.my_computer.mercator_position = self.config_variables_dict["my_computer"]["mercator_position"]
            self.my_computer.graph_position = self.config_variables_dict["my_computer"]["graph_position"]


        ############

        self.settings_panel.interface_label.text = f"""Interface: {self.sniffer_state["sniffing_interface"]} """
    ...
    def update_from_sniffer(self, time_delta: tuple) -> None:
        ...
        for ip in self.sniffer_dictionary.keys():  # iterate over the json dictionary sent from network_sniffer.py
            ...
            if ip in self.ip_dictionary:  # IP, City and Country widgets exist. Just update IP data.
                ...
                # set another timestamp from when the data is processed by visualizer and not when it it seen by sniffer 
                # delay between when the packet is seen by sniffer vs when it is processed by visualizer will cause delay bug. 
                ...
                if (ip in self.malicious_ips_dictionary) or (ip in self.malicious_ips_local_database):
                    ...
                    if ip in self.ip_whois_info_dict.keys():
                        ...
                        cursor = db.cursor()
                        cursor.execute(f"""UPDATE {self.current_connection + '_malicious_ips'} SET description = "{ip_whois_description}" WHERE ip="{ip}" """)
                        db.commit()
                ...
                if country in self.country_dictionary:  # Country widget exists
                    ...
                    if (city in self.country_dictionary[country][1]):  # City widget exists
                        ...
                        if (self.country_dictionary[country][1][city][0].show_ip_widgets == True) and (self.graph == True):
                        ...
                        if country_widget.show_city_widgets == True:
                            ...
                            if city_widget.show_ip_widgets == True and self.graph == True:
    ...
    def set_summary_data(self) -> None:
        ...
        malicious_ips_database_name = self.current_connection + "_malicious_ips"
        ...
        if not self.resolved_whois_data:  # if self.resolved_whois_data dictionary is not empty
            ...
            for ip in self.resolved_whois_data:
                ...
                if (ip in self.malicious_ips_dictionary) or (ip in self.malicious_ips_local_database):
                    ...
                    cursor.execute(f"""UPDATE {self.current_connection + '_malicious_ips'} SET description = "{self.resolved_whois_data[ip]}" WHERE ip="{ip}" """)
                    db.commit()
    ...
    def ip_whois_lookup_thread(self, **kwargs) -> None:
        ...
        for ip_tuple in kwargs["todo_ip_whois_array"]:
            ...
            try:
                ...
                self.live_table_dictionary[ip].children[5].text = ip_whois_description # TODO: use isInstance() or id to search children instead of hard coded description at children[5]

                if self.malicious_table_dictionary[ip]:
                    self.malicious_table_dictionary[ip].children[6].text = ip_whois_description # TODO: use isInstance() or id to search children instead of hard coded description at children[6]
    ...
    def update_table(self, time_delta) -> None:
        ...
        for n, ip in enumerate(self.sniffer_dictionary.keys()):
            ...
            cursor.execute(f"""UPDATE OR IGNORE Live SET total_packets = "{total_packets}", data_in = "{data_in}", data_out = "{data_out}" WHERE ip = "{ip}" """)
    ...
    def update_malicious_table(self, time_delta) -> None:
        ...
        for ip in self.malicious_table_dictionary.keys():
            ...
            if ip not in self.sniffer_dictionary:
                ...
                malicious_ips_database_name = self.current_connection + "_malicious_ips"
                cursor.execute(
                    f"""UPDATE OR IGNORE {malicious_ips_database_name} SET packet_count = "{packet_count}", data_in = "{data_in}", data_out = "{data_out}" WHERE ip = "{ip}" """
                )
    ...
    def sort_table(self, calling_button: Button) -> None:
        ...
        #database need to delete live database 
        ...
        for ip in sorted_ips:
            ...
            try:
                if self.table_count % 2 == 0 and ip[0] not in self.malicious_table_dictionary:
                    ...
                    # BUG: when changing sniffers there is a key error..fixed? 
                ...
                #TODO: remove try/except block when above bug is resolved
                pass
    ...
    def init_database_(self):
        ...
        for stored_connection in self.config_variables_dict["stored_connections"]:
            ...
            stored_connection = self.config_variables_dict["stored_connections"][stored_connection][0].replace(" ", "") 
    ...
    def copy_ip_address(self, button, dropdown) -> None:

        """
        Copy IP address to clipboard
        """

        pyperclip.copy(button.data[0])
        dropdown.dismiss()



    def copy_abuse_emails(self, button, dropdown) -> None:

        """
        Copy IP Abuse emails to clipboard
        """

        pyperclip.copy(button.data[9])
        dropdown.dismiss()
    ...
    def create_malicious_header(self) -> BoxLayout:
        ...
        self.malicious_ip_label_header.on_press = lambda: self.sort_malicious_table( calling_button=self.self.malicious_ip_label_header)
        ...
        ban_lists_button.text = f'[color=FF0000]Ban Lists[/color]   {icon("fa-sort")}'
